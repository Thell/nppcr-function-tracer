...
#>
	...
	PROCESS {
		...
		if (! (Get-FuncGuard -Name $Name -Path $Path -ea 0) ) {
			$NewHeaderContent = createHeaderContent -Name $Name `
					-Prefix $Prefix -Description $Description
			$NewSourceContent = createSourceContent -Prefix $Prefix -Name $Name
			$NewImporterContent = createImporterContent -Name $Name
			...
			if ( $NextGuard -ne $null ) {
				...
				$SourceIndexToken = createSourceContent -Prefix $guard.HeaderPrefix `
					-Name $guard.Name
				$ImporterIndexToken = createImporterContent -Name $guard.Name
			} else {
