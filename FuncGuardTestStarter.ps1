...
# PSUnit default output
#$TestRunner = @(c:\Users\almostautomated\3rdParty\scripts\PSUnit\psunit-2-beta-1-24978\PSUnit.Run.ps1 -Category:$TargetCategory -PSUnitTestFile "C:\scripts\FuncGuard\FuncGuard.test.ps1")

# PSUnit default output plus show browser
#$TestRunner = @(c:\Users\almostautomated\3rdParty\scripts\PSUnit\psunit-2-beta-1-24978\PSUnit.Run.ps1 -Category:$TargetCategory -PSUnitTestFile "C:\scripts\FuncGuard\FuncGuard.test.ps1" -ShowReportInBrowser)

# Raw-Output
$TestRunner = @(%{c:\Users\almostautomated\3rdParty\scripts\PSUnit\psunit-2-beta-1-24978\PSUnit.Run.ps1 -Category:$TargetCategory -PSUnitTestFile "C:\scripts\FuncGuard\FuncGuard.test.ps1" -rawOut})
...
if ($doSummary) {
	$TestRunner | sort Category | group Category |
	%{
	...
	$TestRunner | where {$_.Result -eq "Fail"} | sort Test -Descending | ft -AutoSize
} else {
	$TestRunner | sort Category | group Category | foreach {$_.Group| sort Test } |
	...
	$TestRunner | where {$_.Result -eq "Pass"} | Measure-Object | select -expand count | %{if ($_ -gt 0){"Tests Passed: $($_)"|Write-Output}}
	$TestRunner | where {$_.Result -eq "Fail"} | Measure-Object | select -expand count | %{if ($_ -gt 0){"Tests Failed: $($_)"|Write-Host -ForegroundColor Red}}
	$TestRunner | Measure-Object Result | select -expand count | %{"Tests Run   : $($_)`n"|Write-Output}
}
